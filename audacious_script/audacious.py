...
# white => "00", black => "01", darkblue => "02", darkgreen => "03", lightred => "04",
...
def now_playing(datos, channel, args):
    message = 'esta reproduciendo'
    info_array = get_info_array()
    song = info_array[0]
    length = info_array[1]
    output_length = info_array[2]
    bitrate = info_array[3]
    song_filename = info_array[4]
    if length == '0:00':
        string = "[ %s - %s kbps ]" %(output_length, bitrate)
    else:
        string = "[ %s - %s @ %s kbps ]" %(output_length, length, bitrate)
    source = ''
